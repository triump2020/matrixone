...
func (r *blockMergeReader) loadDeletes(ctx context.Context, cols []string) error {
		...
		for iter.Next() {
			...
			if regexp.MustCompile(`.*sbtest.*`).MatchString(r.table.tableName) {
				logutil.Infof("xxxx txn:%s table:%s, "+
					"block merge reader load deletes:%s,bat:%s from partiont state.",
					r.table.db.txn.op.Txn().DebugString(),
					r.table.tableName,
					entry.RowID.String(),
					common.MoBatchToString(entry.Batch, 10))
			}
		}
		iter.Close()
	} else {
		...
		if (entry.typ == DELETE || entry.typ == DELETE_TXN) && entry.fileName == "" {
			...
			for _, v := range vs {
				...
				if id == info.BlockID {
					...
					if regexp.MustCompile(`.*sbtest.*`).MatchString(r.table.tableName) {
						logutil.Infof("xxxx txn:%s table:%s,"+
							" block merge reader load deletes:%s batch:%s from workspace.",
							r.table.db.txn.op.Txn().DebugString(),
							r.table.tableName,
							v.String(),
	...
	//	logutil.Infof("xxxx txn:%s table:%s, block merge reader load deletes form workspace:%s, bat:%s",
	//		r.table.db.txn.op.Txn().DebugString(),
	//		r.table.tableName,
	//		rowids1)
	//}
