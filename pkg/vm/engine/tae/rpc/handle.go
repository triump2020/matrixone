...
func (h *Handle) HandleWrite(
	...
	if len(req.Batch.Vecs) != 2 {
		panic(fmt.Sprintf("req.Batch.Vecs's length is %d, should be 2, "+
			"one is rowid vector, and the other is pk vector"+
			"handle write type: %v, %d-%s, %d-%s txn: %s, wirte batch: %s",
			len(req.Batch.Vecs),
			req.Type,
			req.TableID,
			req.TableName,
